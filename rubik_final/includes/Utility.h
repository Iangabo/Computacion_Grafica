...
public:
	...
	void Mover_CubitoParabolico(float tiempo) {
		// Cambiar la velocidad y la gravedad según sea necesario
		float velocidad_inicial_y = 5.0f;
		float gravedad = 9.8f;

		if (centro_cubo.y > 0) {
			// Calcular la posición en el eje y usando la ecuación del movimiento parabólico
			float tiempo_actual = tiempo / 1000.0f;  // Convertir tiempo a segundos
			float posicion_y = centro_cubo.y + velocidad_inicial_y * tiempo_actual - 0.5f * gravedad * tiempo_actual * tiempo_actual;

			// Establecer la nueva posición en el eje y
			centro_cubo.y = (posicion_y > 0) ? posicion_y : 0.0f;
		}

		// Mover linealmente en los ejes x y z
		float velocidad_lineal = 1.0f;
		centro_cubo.x += velocidad_lineal * tiempo / 1000.0f;
		centro_cubo.z += velocidad_lineal * tiempo / 1000.0f;

		// Actualizar la matriz de modelo
		modelo = glm::translate(glm::mat4(1.0), centro_cubo);			
	}
...
public:
	...
	void Parabola(float tiempo) {
		for (size_t i = 0; i < cubitos.size(); ++i) {
			if (i != 1) {
				cubitos[i]->Mover_CubitoParabolico(tiempo);
			}
		}
	}
    ...
    void Mover() {
        ...
        int axis;
		bool esHorario = AnimacionActual.length() == 2 && AnimacionActual[1] == '\'';
